...
**Note:** `SALESFORCE_INSTANCE_URL` and `SALESFORCE_AUTH_CODE` are now provided dynamically through the MCP interface and are no longer needed as environment variables.
...
3. **Configure your Salesforce connection:**
   Edit `.env` file:
   ```bash
   # Direct Token authentication (fallback method)
   SALESFORCE_ACCESS_TOKEN=your_access_token_here
   ...
   **Note:** For HTTP server usage, `SALESFORCE_INSTANCE_URL` and `SALESFORCE_AUTH_CODE` should be provided dynamically via the API requests, not as environment variables.
